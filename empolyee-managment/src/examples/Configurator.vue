...
// mutations
const navbarFixed = () => store.commit("layout/navbarFixed");
const setSidebarType = (type) => store.commit("layout/sidebarType", type);
...
const darkMode = () => {
  if (store.state.layout.darkMode) {
    store.state.layout.darkMode = false;
    setSidebarType("bg-white");
    deactivateDarkMode();
    return;
  } else {
    store.state.layout.darkMode = true;
        ...
        <div class="mt-2 mb-5 d-flex">
          ...
          <div class="form-check form-switch ps-0 ms-auto my-auto">
            <input
              ...
              type="checkbox"
              :checked="store.state.layout.darkMode"
              @click="darkMode"
            />
          </div>
        </div>
